...
class WeightEMAI:
    ...
    The estimator currently supports the V-matrix formulation (`fit_vmat`).
    """
...
def _read_grm_ids(grm_prefix):
    ...
    Supports:
    - <prefix>.id (new output from gmatrix.py, usually IID or FID IID)
    - <prefix>.agrm.id (legacy format, usually IID only)
    """
    base = _normalize_grm_prefix(grm_prefix)
    id_file = _find_first_existing_file(
        [f"{base}.id", f"{base}.agrm.id"],
        "GRM ID file",
    )
...
def _load_grm_matrix(grm_prefix, n_id):
    ...
    Supported formats:
    - *.grm.mat_fmt / *.agrm.mat_fmt
    - *.grm.ind_fmt / *.agrm.ind_fmt (lower-triangle row/col/value)
    """
    base = _normalize_grm_prefix(grm_prefix)
    mat_file_candidates = [
        f"{base}.grm.mat_fmt",
    ...
    raise FileNotFoundError(
        "Unable to find GRM matrix file. "
        f"Tried mat formats: {mat_file_candidates}; ind formats: {ind_file_candidates}"
    )
...
def prepare_varcom_inputs(
    ...
    grm_prefix : str
        ...
        Example: if files are out.id and out.grm.mat_fmt, grm_prefix should be "out".
